...
class Wiki_mcp {
	...
	public function index()
	{
		...
		$vars['cp_page_title'] = lang('wiki_module_name');
	...
	public function update()
	{
		...
		ee()->javascript->set_global(array('lang' => array(
				'namespace_deleted'     => lang('namespace_deleted'),
				'namespace_not_deleted' => lang('namespace_not_deleted')
		)));
		...
		{
			$message = lang('wiki_created');
			$vars['message'] = $message;
			$vars['message_type'] = 'success';
		}

		ee()->cp->set_breadcrumb($this->base_url, lang('wiki_module_name'));

		$vars['cp_page_title'] = lang('wiki_preferences');
		...
		if (ee()->form_validation->run() === FALSE)
		{
			...
			// Namespaces
			$namespaces_query = ee()->db->get_where(
				'wiki_namespaces',
		...
		else
		{
					...
					if ($query->num_rows() > 0)
					{
						foreach($query->result_array() as $row)
						{
							if (isset($_POST['namespace_label_'.$row['namespace_id']]) && isset($_POST['namespace_name_'.$row['namespace_id']]))
							{
								...
								{
									return ee()->output->show_user_error('submission', array(lang('invalid_namespace')));
								}
			...
			if (count($data) > 0)
			{
				...
				ee()->session->set_flashdata('message_success', lang('update_successful'));
			}
	...
	/**
	  ...
	  */
	public function _check_duplicate($str)
	{
		ee()->load->model('wiki_model');

		if (ee()->wiki_model->check_duplicate(ee()->form_validation->old_value('id'), $str) === FALSE)
		{
			ee()->form_validation->set_message('_check_duplicate', lang('duplicate_short_name'));
			return FALSE;
		}
	...
	public function delete_confirm()
	{
		...
		$vars['cp_page_title'] = lang('wiki_delete_confirm');
	...
	public function list_themes()
	{
		...
		$vars = array();
		$vars['cp_page_title'] = lang('wiki_themes');
		$vars['themes'] = array();
	...
	{
		ee()->cp->set_breadcrumb($this->base_url, lang('wiki_module_name'));
		ee()->cp->set_breadcrumb($this->base_url.AMP.'method=list_themes', lang('wiki_themes'));

		$vars = array();
		$vars['cp_page_title'] = lang('wiki_theme_templates');
	...
	public function edit_template()
	{
		...
		$vars = array();
		$vars['cp_page_title'] = lang('edit_template');
		...
		ee()->cp->set_breadcrumb(
			$this->base_url.AMP.'method=theme_templates'.AMP.'theme='.$vars['theme'],
	...
	public function update_template()
	{
		...
		{
			show_error(lang('invalid_wiki_theme'));
		}

		// no template?
		if (($template = ee()->input->get_post('template')) === FALSE)
		{
			show_error(lang('invalid_template'));
		}
		...
		ee()->session->set_flashdata('message_success', lang('template_updated'));
