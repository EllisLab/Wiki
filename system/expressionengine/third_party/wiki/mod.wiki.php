...
class Wiki {
	...
	public $profile_path           = '';
	public $base_url               = '';
	public $seg_parts              = array();
	public $admins                 = array('1');
	public $users                  = array('1', '5');
	...
	//  Namespaces
	public $main_ns                = 'Main';
	public $special_ns             = 'Special';  // Deutsch: Spezial
	public $file_ns                = 'File';
	public $category_ns            = 'Category'; // Deutsch: Kategorie
	public $image_ns               = 'Image';
	public $current_namespace      = '';
	public $namespaces             = array();

	// Settings
	public $wiki_id                = 1;
	public $label_name             = 'EE Wiki';
	public $use_captchas           = 'n';
	public $text_format            = 'xhtml';
	public $html_format            = 'safe';
	public $auto_links             = "n";
	public $upload_dir             = '';
	public $valid_upload_dir       = 'n';
	public $can_upload             = 'n';
	public $moderation_emails      = '';
	public $revision_limit         = 100;
	public $author_limit           = 75;
	public $min_length_keywords    = 3;
	public $cache_expire           = 2;  // How many hours should we keep wiki search caches?
	public $cats_separator         = '::';
	public $cats_display_separator = ' -> ';
	public $cats_use_namespaces    = 'n';
	public $cats_assign_parents    = 'y';

	// Category Retrieval
	public $temp_array             = array();
	public $cat_array              = array();
	public $show_these             = FALSE;
	public $cat_depth              = 0;
	public $parent_cats            = array();
	...
	function create_url($namespace='', $title)
	{
		...
		{
			$link = ee()->functions->create_url(
				$this->base_path.urlencode($this->valid_title($title))
			);
		}
	...
	function prep_title($str)
	{
		...
		{
			return str_replace(
				array('-', $this->cats_separator),
				array(' ', $this->cats_display_separator),
				$str
			);
		}
		else
	...
	function file($topic)
	{
		...
		$summary = $this->convert_curly_brackets(
			ee()->typography->parse_type(
				$this->wiki_syntax($query->row('upload_summary')),
				array(
		...
		$this->return_data = str_replace(
			array('{file_url}','{delete_url}','{file_name}','{summary}', '{image_width}', '{image_height}', '{file_type}'),
		...
		foreach($results->result_array() as $row)
		{
			...
			if ($parse_article !== FALSE)
			{
				$data['{article}'] = $this->convert_curly_brackets(
					ee()->typography->parse_type(
						$this->wiki_syntax($row['page_content'], FALSE),
						array(
							'text_format'   => $this->text_format,
							'html_format'   => $this->html_format,
							'auto_links'    => $this->auto_links,
	...
	function category_process($template, $data, $parents, $depth, $last_depth='0', $next_depth='0', $children='n', $style='')
	{
		...
		$cdata = array(
			'{category_name}'      => $this->prep_title($data['cat_name']),
			'{category_id}'        => $data['cat_id'],
			'{parent_id}'          => $data['parent_id'],
			'{depth}'              => $depth,
			'{last_depth}'         => $last_depth,
			'{next_depth}'         => $next_depth,
			'{path:view_category}' => $this->base_url.$this->category_ns.':'.
	...
	function edit($title)
	{
		...
		*/
		...
		else
		{
			...
			$preview = $this->convert_curly_brackets(
				ee()->typography->parse_type(
					$this->wiki_syntax($_POST['article_content']),
					array(
						'text_format'   => $this->text_format,
						'html_format'   => $this->html_format,
						'auto_links'    => $this->auto_links,
	...
	function edit_revision($revision_id, $title)
	{
		...
		$this->return_data = str_replace(
			array('{form_declaration:wiki:edit}', '{content}', '{redirect_page}', '{revision_notes}', '{rename}'),
			array(ee()->functions->form_declaration($data), $this->encode_ee_tags(form_prep($content)), $redirect, $revision_notes, ''),
			$this->return_data
		);
	}
		...
		if (preg_match("/\{wiki:revisions.*?\}(.*?)\{\/wiki:revisions\}/s", $this->return_data, $match))
		{
			...
			else
			{
				...
				foreach ($results->result_array() as $row)
				{
					...
					$data = array(
						'{revision_author}'     => $this->prep_screen_name($row['screen_name']),
						'{revision_notes}'      => $row['revision_notes'],
	...
	function new_article($title, $original_page='')
	{
		...
		if ($original_page != '')
		{
			...
			$this->return_data = str_replace(
				array('{original_page}', '{path:original_page}'),
		...
		if ($this->current_namespace == $this->category_ns
			&& (stristr($this->return_data, '{/wiki:category_subcategories}')
				OR stristr($this->return_data, '{wiki:category_articles}'))
			)
		{
			$this->category_page();
		}
	...
	function article($title)
	{
		...
		*/
		...
		if ($query->row('page_redirect')  != '' && preg_match("|".preg_quote($title)."/noredirect|i", ee()->uri->uri_string, $url))
		{
			...
			$this->return_data = str_replace(
				array('{author}', '{article}', '{content}', '{redirect_page}', '{path:redirect_page}'),
		...
		$article = $this->convert_curly_brackets(
			ee()->typography->parse_type(
				$this->wiki_syntax($results->row('page_content')),
				array(
					'text_format'   => $this->text_format,
					'html_format'   => $this->html_format,
					'auto_links'    => $this->auto_links,
		...
		$this->return_data = str_replace(
			array('{author}', '{article}', '{content}'),
			array($results->row('screen_name') , $article, $results->row('page_content') ),
			$this->return_data
		);
	...
	function parse_dates($str)
	{
		...
		if (preg_match_all("/".LD."(gmt_last_updated|gmt_revision_date|last_updated|revision_date)\s+format=[\"'](.*?)[\"']".RD."/s", $this->return_data, $matches))
		{
			for ($j = 0; $j < count($matches['0']); $j++)
			{
				switch ($matches['1'][$j])
				{
					case 'gmt_last_updated':
						$dates['gmt_last_updated'][$matches['0'][$j]] = $matches['2'][$j];
						break;
					case 'last_updated':
						$dates['last_updated'][$matches['0'][$j]] = $matches['2'][$j];
						break;
					case 'gmt_revision_date':
						$dates['gmt_revision_date'][$matches['0'][$j]] = $matches['2'][$j];
						break;
					case 'revision_date':
						$dates['revision_date'][$matches['0'][$j]] = $matches['2'][$j];
						break;
				}
			}
		}
	...
	function revision($title)
	{
		...
		$this->return_data = str_replace(
			...
			array($article, $results->row('page_content')),
			$this->return_data
		);
	}
	...
	function edit_article()
	{
		...
		if ($revision['revision_status'] == 'closed' && trim($this->moderation_emails) != '')
		{
			...
			$revision['article'] = $this->convert_curly_brackets(
				ee()->typography->parse_type(
					$this->wiki_syntax(ee()->security->xss_clean($content)),
					array(
						'text_format'   => $this->text_format,
						'html_format'   => $this->html_format,
						'auto_links'    => $this->auto_links,
		...
		*/
		...
		$query = ee()->db->query("SELECT COUNT(revision_id) AS count
			FROM exp_wiki_revisions
		...
		{
			$query = ee()->db->query("SELECT revision_id
				FROM exp_wiki_revisions
				WHERE page_id = '".ee()->db->escape_str($page_id)."'
				AND wiki_id = '".ee()->db->escape_str($this->wiki_id)."'
			...
			for($i=0, $s = count($matches['0']); $i < $s; ++$i)
			{
				...
				if (stristr($matches['2'][$i], $this->cats_separator))
				{
					$cats = explode(
						$this->cats_separator,
						preg_replace(
							"/".preg_quote($this->cats_separator.$this->cats_separator)."+/",
							$this->cats_separator,
							$matches['2'][$i]
						)
					);
				...
				{
					$data = array(
						'cat_name'      => $this->valid_title($cats['0']),
						'parent_id'     => 0,
						'wiki_id'       => $this->wiki_id,
						'cat_namespace' => ''
					);
	...
	function upload_form()
	{
		...
		$this->return_data = str_replace(
			array('{form_declaration:wiki:uploads}', '{file_types}'),
	...
	function edit_limit()
	{
		if ( ! in_array(ee()->session->userdata['group_id'], $this->admins))
		{
			...
			$query2 = ee()->db->query("SELECT COUNT(wiki_upload_id) AS count
				FROM exp_wiki_uploads
	...
	function open_close_revision($title, $revision_id, $new_status)
	{
		if (in_array(ee()->session->userdata['group_id'], $this->admins))
		{
			$query = ee()->db->query("SELECT r.page_id, r.page_content, p.page_namespace
	...
	function display_attachment()
	{
		...
		$query = ee()->db->query("SELECT file_name, image_width, image_height, file_type, file_hash
			FROM exp_wiki_uploads
					...
					switch($x['0'])
					{
						case $this->category_ns:
							if (($pipe_pos = strpos($matches['1'][$i], '|')) !== FALSE)
							{
								...
								$matches['1'][$i] = '[url="'.$this->base_url.$title.'" title="'.$title.'"]'.$this->prep_title($display).'[/url]';
							}
							else
							{
								$matches['1'][$i] = '[url="'.$this->base_url.$this->category_ns.':'.$title.'" title="'.$this->category_ns.':'.$title.'"]'.$this->category_ns.':'.$this->prep_title($title).'[/url]';
							}
							break;
